...
class Sweep(Animation):
    def setup(self, locations, **kwargs):
    ...
    def run(self):
        ...
        while not self._stop_event.is_set():
            ...
            while(idx<num_leds and self.check(self.locs[idx],loc)): #todo take invert into account
                self.strip.setPixelColor(int(self.order[idx]), self.color(loc, self.max_loc, self.iteration))
                changed = True
                idx += 1
            if(changed):
                self.strip.show()
...
class Sweep_Vertical(Sweep):
    ...
    def setup(self, **kwargs):
        super().setup(get_locations()[:, 2], **kwargs)
        self._is_setup = True
        return {"success": True}
...
class Sweep_Horizontal(Sweep):
    ...
    def setup(self, **kwargs):
        ...
        super().setup(locs, **kwargs)
        self._is_setup = True
        return {"success": True}

class SweepX(Sweep):
    def setup(self, **kwargs):
        super().setup(get_locations()[:, 0], **kwargs)
        self._is_setup = True
        return {"success": True}